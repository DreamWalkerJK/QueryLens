...
public partial class MainViewModel : ViewModelBase
{
    ...
    [ObservableProperty] private bool confirmRetention;
    ...
    partial void OnSelectedQueryChanged(SlowQuery? value)
    {
        ...
        PlanQueryFingerprint = value?.Fingerprint ?? "";
        ApplyQueryFilters();
    }
    ...
    { try { EnsureStore(); var previous = SelectedConnection; var (p, temporary) = await BuildProfileAsync(token); await _store!.SaveConnectionAsync(p, token); if (temporary is not null && _secrets is not null) await _secrets.DeleteAsync(temporary, token); if (previous is not null && previous.Id == p.Id && previous.SecretReference is not null && previous.SecretReference != p.SecretReference && _secrets is not null) await _secrets.DeleteAsync(previous.SecretReference, token); SelectedConnection = p; await ReloadAsync(token); StatusMessage = $"连接“{p.Name}”已保存；密码只保存为受保护引用。"; } catch (Exception ex) { ErrorMessages.Insert(0, $"{DateTimeOffset.Now:T} 保存连接：{ex.Message}"); StatusMessage = UserMessage(ex); } }
    [RelayCommand] private async Task TestConnectionAsync(CancellationToken token)
    { string? temporary = null; try { if (OfflineMode) { StatusMessage = "当前为离线样例模式，未发起网络连接。关闭离线模式后再测试。"; return; } var built = await BuildProfileAsync(token, persistSecret: false); temporary = built.TemporarySecretReference; var p = built.Profile; var a = DatabaseAdapterFactory.Create(p.Dialect, _secrets); var info = await a.DetectAsync(p, token); var caps = await a.GetCapabilitiesAsync(p, token); Capabilities.Clear(); foreach (var c in caps) Capabilities.Add(c); StatusMessage = $"已连接 {info.Product} {info.Version}；能力 {caps.Count} 项。"; } catch (Exception ex) { StatusMessage = UserMessage(ex); } finally { if (temporary is not null && _secrets is not null) await _secrets.DeleteAsync(temporary, default); } }
    [RelayCommand(IncludeCancelCommand = true)] private async Task CollectAsync(CancellationToken token)
    { string? temporary = null; try { EnsureStore(); if (OfflineMode) { await LoadOfflineSamplesAsync(token); return; } var built = await BuildProfileAsync(token, persistSecret: false); temporary = built.TemporarySecretReference; var p = built.Profile; var rows = await DatabaseAdapterFactory.Create(p.Dialect, _secrets).ReadSlowQueriesAsync(p, limit: 500, cancellationToken: token); var snapshotId = Guid.NewGuid(); var snapshot = new SlowQuerySnapshot(new SnapshotMetadata(snapshotId, p.Id, p.Dialect, "live-collection", DateTimeOffset.UtcNow, 500, rows.Count < 500, p.Database, null, null, false, false), rows.Select(r => r with { SnapshotId = snapshotId }).ToArray()); await _store!.SaveSnapshotAsync(snapshot, token); await ReloadAsync(token); StatusMessage = $"已采集 {rows.Count} 条统计并保存快照；累计计数器不会被当成窗口平均值。"; } catch (Exception ex) { StatusMessage = UserMessage(ex); } finally { if (temporary is not null && _secrets is not null) await _secrets.DeleteAsync(temporary, default); } }
    ...
    [RelayCommand] private async Task SaveRetentionAsync(CancellationToken token) { try { EnsureStore(); if (!ConfirmRetention) { StatusMessage = $"请先勾选确认，再清理所有连接中早于 {RetentionDays} 天的非基线记录。"; return; } if (RetentionDays is < 0 or > 3650) throw new ArgumentOutOfRangeException(nameof(RetentionDays), "保留天数必须为 0–3650。"); await _store!.SaveSettingAsync("retention_days", RetentionDays.ToString(), token); var removed = await _store.ApplyRetentionAsync(RetentionDays, token); ConfirmRetention = false; await ReloadAsync(token); StatusMessage = $"已保存保留策略：{RetentionDays} 天；清理 {removed} 条记录。"; } catch (Exception ex) { ErrorMessages.Insert(0, $"{DateTimeOffset.Now:T} 保留策略：{ex.Message}"); StatusMessage = $"保留策略失败：{ex.Message}"; } }
    ...
    private async Task ReloadAsync(CancellationToken token = default) { EnsureStore(); var beforeId = _beforePlan?.Id; _isReloading = true; try { await BuildConnectionListAsync(token); Queries.Clear(); foreach (var q in await _store!.GetSlowQueriesAsync(OfflineMode ? null : SelectedConnection?.Id, 1000, token)) if (q.IsOfflineSample == OfflineMode) Queries.Add(q); ApplyQueryFilters(); Plans.Clear(); foreach (var p in await _store.GetPlansAsync(OfflineMode ? null : SelectedConnection?.Id, 500, token)) if (p.IsOfflineSample == OfflineMode) Plans.Add(p); _beforePlan = beforeId.HasValue ? Plans.FirstOrDefault(p => p.Id == beforeId.Value) : Plans.FirstOrDefault(p => p.IsBaseline); } finally { _isReloading = false; } }
    private async Task<(ConnectionProfile Profile, string? TemporarySecretReference)> BuildProfileAsync(CancellationToken token, bool persistSecret = true) { if (!int.TryParse(Port, out var p) || p is < 1 or > 65535) throw new ArgumentException("端口必须为 1–65535。", nameof(Port)); if (TimeoutSeconds is < 1 or > 300) throw new ArgumentException("超时必须为 1–300 秒。", nameof(TimeoutSeconds)); var r = SelectedConnection?.SecretReference; string? temporary = null; if (!string.IsNullOrEmpty(PasswordInput)) { if (_secrets is null) throw new PlatformNotSupportedException("当前系统没有可用的受保护凭据存储。"); r = await _secrets.SaveAsync(PasswordInput, token); if (!persistSecret) temporary = r; PasswordInput = ""; } var auth = AuthenticationText switch { "集成认证" => AuthenticationMode.Integrated, "令牌" => AuthenticationMode.Token, _ => AuthenticationMode.Password }; return (new(SelectedConnection?.Id ?? Guid.NewGuid(), ConnectionName.Trim(), ParseDialect(), Host.Trim(), p, Database.Trim(), auth, UseTls, TimeoutSeconds, string.IsNullOrWhiteSpace(UserName) ? null : UserName.Trim(), r, ProductVariant.Trim()), temporary); }
